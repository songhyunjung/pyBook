        ...
        if isValid:
            ...
            if cursor:
                if files:
                    ...
                    SQL = 'INSERT INTO articles (author, title, category, description, price) VALUES (?,?,?,?,?)'
...
def getRecentArticles():
    ...
    if len(result) > 0:
        for article in result:
            recentArticleDics.append({"articleNo": article[0], "title": article[2],
                                      "desc": article[4]})
...
def displayArticlePage(ariticleNo):
...
def searchArticles():
    ...
    if cursor:
        SQL = ('SELECT articleNo, author, title, category, description, price, picture FROM articles WHERE title LIKE '
               '"%{skwd}%" ORDER BY articleNo DESC').format(skwd=searchKeyword)
        cursor.execute(SQL)
        result = cursor.fetchall()
